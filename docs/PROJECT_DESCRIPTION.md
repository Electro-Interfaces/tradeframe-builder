...
> ⚠️ Документ частично устарел. Для текущего технического состояния версии 2.1.3 используйте `docs/HANDOVER.md` и `docs/ARCHITECTURE_CURRENT.md`. Этот файл полезен как продуктовый обзор, но версии, статистика и отдельные маршруты требуют сверки.
