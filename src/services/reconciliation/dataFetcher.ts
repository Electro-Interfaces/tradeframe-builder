...
export async function getShiftsWithReports(
  ...
  const stationsToQuery = normalizedStationIds.length > 0 ? normalizedStationIds : await getAvailableStations();
    ...
    try {
      ...
      for (const shift of shifts) {
        try {
          ...
          // Извлекаем данные по корп.картам из sales
          ...
          if (shiftReport?.sales) {
            for (const sale of shiftReport.sales) {
              ...
              // Проверяем тип оплаты - корпоративные карты (КР)
 ...
 */
export async function getAvailableStations(): Promise<number[]> {
  try {
