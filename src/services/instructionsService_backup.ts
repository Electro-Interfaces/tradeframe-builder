...
const initialVersions: InstructionVersion[] = [
  // Обзор сети
  {
    ...
    status: 'published',
    locale: 'ru',
    title: 'Обзор сети - руководство пользователя',
...
- **Переход к детальной информации** - клик по виджетам для перехода к соответствующим разделам`,
  ...
  // Оборудование
  {
    ...
    status: 'published',
    locale: 'ru',
    title: 'Управление оборудованием АЗС',
    content_md: `# 🔧 Управление оборудованием АЗС

> **Система мониторинга и управления оборудованием** торговой точки в режиме реального времени с интеграцией STS API

---

## 🎯 Назначение страницы

Страница **Оборудование** обеспечивает комплексный контроль всех систем АЗС:

- 🖥️ **Мониторинг терминального оборудования** (АЗС, POS, QR-системы)
- 💰 **Контроль купюроприемника** с отображением наличных средств
- 🗂️ **Управление картридерами** и МПС-ридерами
- 🛢️ **Отслеживание состояния резервуаров** с уровнями топлива
- ⚡ **Автоматическое обновление** данных из STS API

---

## 🏗️ Структура интерфейса

### 📊 Панель терминального оборудования

**Отображаемые устройства:**
- **АЗС** - основной терминал станции с онлайн статусом
- **POS** - терминал продаж с версией ПО
- **QR** - система QR-платежей с информацией о смене
- **Купюроприемник** - устройство приема наличных с детальной статистикой
- **Картридер** - терминал банковских карт  
- **МПС-ридер** - ридер мобильных платежных систем

### 🛢️ Раздел резервуаров

**Информация по каждому резервуару:**
- Тип топлива (АИ-92, АИ-95, ДТ)
- Текущий уровень в литрах и процентах заполнения
- Температура продукта
- Уровень подтоварной воды
- Статус датчиков
- Прогресс-бар заполнения с цветовой индикацией

---

## 💰 Специальные возможности купюроприемника

### Расширенный мониторинг:
- **Количество купюр** - общее количество принятых банкнот
- **Сумма наличных** - общая стоимость в рублях
- **Статус готовности** - техническое состояние устройства
- **ID устройства** - идентификатор для технического обслуживания

**Пример отображения купюроприемника:**
- 💵 **Купюр:** 341 шт
- 💰 **Сумма:** 153,450 ₽  
- 🏷️ **ID:** Устройство 10
- ✅ **Статус:** Готов
...
## 🛢️ Система мониторинга резервуаров

### Цветовая индикация уровней:
- 🟢 **НОРМА** (70%+) - достаточный запас топлива
- 🟡 **ВНИМАНИЕ** (40-70%) - средний уровень
- 🔴 **НИЗКИЙ** (<40%) - требуется пополнение

### Детальная информация:
- **Объем емкости** - максимальная вместимость резервуара
- **Текущий уровень** - фактическое количество топлива
- **Температура** - важный параметр для качества топлива
- **Вода** - контроль подтоварной воды (должна быть минимальной)

### Датчики мониторинга:
- 🔵 **Датчик уровня** - измерение объема топлива
- 🔴 **Датчик температуры** - контроль температурного режима  
- 🟢 **Датчик воды** - определение наличия подтоварной воды
...
**STS API интеграция:**
- Данные обновляются автоматически при выборе торговой точки
- Конфигурация подключения к \`https://pos.autooplata.ru/tms\`
- Использование учетных данных: \`UserApi\` / \`lHQfLZHzB3tn\`
- Таймаут операций: 30 секунд, до 3 попыток

### 📱 Адаптивный дизайн

**Мобильная версия:**
- Компактное отображение карточек оборудования
- Сенсорное управление
- Оптимизированные размеры текста и элементов
- Скрытие второстепенных элементов на малых экранах

**Десктоп версия:**
- Полная таблица резервуаров с детальными данными
...
- Кнопка **"Обновить STS данные"** для принудительной синхронизации
- Автоматические уведомления об успешной загрузке (только десктоп)
- Обработка ошибок с отображением демо-данных при сбоях
...
1. **Утренняя проверка:**
   - Просмотр статусов всего оборудования
   - Контроль уровней топлива в резервуарах
   - Проверка работы купюроприемника

2. **Мониторинг в течение дня:**
   - Отслеживание изменений уровней топлива
   - Контроль работы платежных систем
   - Мониторинг температуры в резервуарах

3. **Вечерние операции:**
   - Проверка накопленной суммы в купюроприемнике
   - Анализ расхода топлива за день
   - Планирование пополнения резервуаров

### ⚠️ Критические ситуации:

**При низких уровнях топлива:**
- Статус резервуара изменится на "Мало" или "Критично"
- Необходимо срочно запланировать поставку
- Связаться с поставщиком топлива

**При ошибках оборудования:**
- Проверить физическое состояние устройств
- Обратиться в техническую поддержку
- При необходимости временно отключить неисправное оборудование

**При сбоях связи:**
- Система автоматически переключится на демо-данные
- Проверить интернет-соединение на АЗС
- Убедиться в доступности STS API сервера
...
**💡 Рекомендации:**
- Регулярно проверяйте страницу в начале смены
- Обращайте внимание на цветовую индикацию резервуаров
- Следите за показателями купюроприемника для планирования инкассации
- При появлении ошибок оборудования немедленно принимайте меры

**🔍 Диагностика проблем:**
- Красный статус оборудования = требует внимания
- Низкий уровень топлива = планировать поставку
- Ошибка подключения = проверить связь с STS API

**📊 Аналитика:**
- Используйте историю данных для прогнозирования расхода
- Анализируйте эффективность работы оборудования
- Планируйте техническое обслуживание на основе статистики`,
    content_html: '',
    changelog: 'Полная инструкция по работе с оборудованием АЗС',
    editor_id: 'admin',
    editor_name: 'Администратор',
...
**Пример панели остатков топлива:**
- 🟢 **АИ-92:** 2,847 т (↗️ +127 т за сутки)
- 🔵 **АИ-95:** 1,923 т (↘️ -54 т за сутки)
- 🟡 **АИ-98:** 486 т (→ +12 т за сутки)
- ⚫ **ДТ:** 3,294 т (↗️ +203 т за сутки)
- 🔴 **АИ-100:** 156 т (↘️ -23 т за сутки)
