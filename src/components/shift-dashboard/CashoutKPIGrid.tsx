...
function KPICard({
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <div className="h-5 sm:h-7 w-20 sm:w-28 bg-secondary rounded animate-pulse" />
      ) : (
    ...
    <div className={cn('space-y-3 sm:space-y-4', className)}>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <KPICard
          ...
          icon={<Wallet className="w-3 h-3 sm:w-4 sm:h-4 text-foreground" />}
        ...
        >
          <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            <div className="p-1 sm:p-1.5 rounded-lg bg-purple-600">
              <Banknote className="w-3 h-3 sm:w-4 sm:h-4 text-foreground" />
            </div>
            <span className="text-xs sm:text-sm font-medium text-foreground">Кол-во</span>
          </div>
          {isLoading ? (
            <div className="h-5 sm:h-7 w-12 sm:w-16 bg-secondary rounded animate-pulse" />
          ) : (
            <div className="flex items-baseline gap-1 sm:gap-2">
              <span className="text-base sm:text-xl font-bold text-foreground">{safeCashout.count}</span>
              <span className="text-[10px] sm:text-xs text-muted-foreground hidden sm:inline">инкассаций</span>
            </div>
          )}
        </div>
      </div>
            ...
            <table className="w-full text-xs sm:text-sm">
              ...
              <tbody className="divide-y divide-border">
                {safeCashout.details.map((item, idx) => (
                  <tr key={`${item.shiftNumber}-${item.posNumber}-${idx}`} className="hover:bg-secondary/30">
