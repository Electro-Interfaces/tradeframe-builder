...
/**
 * Строка-расшифровка бонусов и скидок («Спасибо», «Дисконт» на АЗС Н1).
 * Это НЕ отдельный отпуск: литры уже учтены в основном способе оплаты
 * (бонусы «Спасибо» списываются по карте Сбера), здесь показано только,
 * какая часть чека закрыта деньгами, а какая — бонусами или скидкой.
 */
export interface BonusBreakdownItem {
  /** Название разреза («Спасибо», «Дисконт») */
  payTypeName: string;
  ...
  /** Доплачено деньгами (рубли) */
  paidCash: number;

  /** Покрыто бонусами или скидкой (рубли) */
  paidBonus: number;
}
...
export interface ReceiptItem {
  ...
  /** Код нефтебазы в учётной системе АЗС (base.id из STS) */
  supplierCode?: number;
}
...
export interface ShiftDetails extends ShiftListItem {
  ...
  /** Расшифровка бонусов и скидок (пусто там, где STS их не отдаёт) */
