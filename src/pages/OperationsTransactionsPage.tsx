    ...
    <MainLayout>
      <div className={`${isMobile ? 'w-full' : 'fixed top-16 left-64 right-0 bottom-0 overflow-y-auto'}`}>
        ...
        <div className="mb-6 px-6 pt-4">
          <h1 className="text-2xl font-semibold text-white">Операции и транзакции</h1>
          <p className="text-slate-400 mt-2">
            {isNetworkOnly && "Отчет по операциям и транзакциям торговой сети"}
            {isTradingPointSelected && "Отчет по операциям и транзакциям торговой точки"}
            {!selectedNetwork && "Выберите сеть для просмотра операций и транзакций"}
          </p>
        </div>

        {/* Карточка: Операции и транзакции */}
        <div className="bg-slate-800 border border-slate-700 rounded-lg shadow-lg">
          <div className="px-6 py-4 border-b border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-white">Отчет по операциям и транзакциям</h2>
            </div>
          </div>

          <div className="p-6">
            <div className="text-slate-400 text-center py-8">
              Здесь будет отчет по операциям и транзакциям
            </div>
          </div>
        </div>
        </div>
