...
async function fetchAllRows(supabase, table, orderColumn, filters = []) {
  const pageSize = 1000;
  const rows = [];
  let from = 0;
...
async function main() {
  ...
  const data = await fetchAllRows(supabase, 'tenants', 'name', [
    { type: 'eq', column: 'type', value: 'network' },
  ]);
  ...
  console.log(`Экспортировано сетей: ${data?.length || 0}`);
